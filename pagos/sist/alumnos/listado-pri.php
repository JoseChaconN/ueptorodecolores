...
if($gradoVer=='0' || $secciVer=='0')
{
    ...
    $query = mysqli_query($link,"SELECT A.idAlum, D.statusAlum, A.cedula as 'cedalu',A.nombre as 'nomalu', A.ced_rep,A.id_quienPaga, B.nombreGrado as 'nomgra', A.apellido, D.grado, D.idSeccion, D.pagado, C.nombre as 'nomsec' FROM alumcer A, grado".$tablaPeriodo." B, secciones C, notaprimaria".$tablaPeriodo." D WHERE A.idAlum=D.idAlumno and 
...
{
    $query = mysqli_query($link,"SELECT A.idAlum, D.statusAlum, A.cedula as 'cedalu',A.nombre as 'nomalu', A.ced_rep,A.id_quienPaga, B.nombreGrado as 'nomgra', A.apellido, D.grado, D.idSeccion,D.pagado, C.nombre as 'nomsec' FROM alumcer A, grado".$tablaPeriodo." B, secciones C, notaprimaria".$tablaPeriodo." D WHERE A.idAlum=D.idAlumno and D.grado='$gradoVer' and D.idSeccion='$secciVer' and D.grado=B.grado and D.idSeccion=C.id ORDER BY D.grado,D.idSeccion, A.apellido ASC ");
}
...
<div class="container-fluid">
    ...
    <div class="form-row">
        <div class="col-md-6">
            <h1 class="h3 mb-2 text-gray-800">Listado de Estudiantes (Primaria)</h1>    
        </div>    
        <div class="col-md-3">
            <button type="button" class="btn btn-primary" onclick='window.open("listado-pri-pdf.php?idG=<?= $gradoVer ?>&idS=<?= $secciVer ?>&peri=<?= $tablaPeriodo ?>&nomP=<?= $nombre_periodo ?>")' style="width: 100%"><i class="fas fa-print"></i> Imprimir Listado</button>
        </div>
        <div class="col-md-3">
            <button type="button" class="btn btn-success" onclick='window.open("perfil-pri-alumno.php?guarda=nuevo&peri=<?= $tablaPeriodo.'&gra='.$gradoVer.'&sec='.$secciVer.'&nomP='.$nombre_periodo ?>")' style="width: 100%"><i class="fas fa-plus"></i> Nuevo Alumno</button>
        </div>
    </div>
    <!-- DataTales Example -->
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <form role="form" method="POST" enctype="multipart/form-data" action="listado-pri.php">
                    <div class="form-row">
                        ...
                        <div class="col-md-3 col-md-offset-3 col-xs-6 col-sm-6">
                            <select name="gradoVer" class="form-control" onchange="pulsaBuscar()">                              
                                ...
                                $gradoVer_query = mysqli_query($link,"SELECT grado, nombreGrado FROM grado".$tablaPeriodo." WHERE grado<61 ORDER BY grado ");
                    ...
                    <tbody><?php 
                        ...
                        while($row=mysqli_fetch_array($query)) 
                        {
                            ...
                            <tr id="linea<?= $son ?>">
                                ...
                                <td style='cursor: pointer' onclick='window.open("perfil-pri-alumno.php?id=<?= encriptar($idAlum).'&peri='.$tablaPeriodo.'&gra='.$gradoVer.'&sec='.$secciVer.'&nomP='.$nombre_periodo ?>")'><?= substr($row['apellido'], 0,20); ?></td>
                                <td><?= $nomgra.' "'.$nomsec.'"' ?></td> <?
                                ...
                                <td>
                                    <div class="dropdown mb-4 btn-group">
                                        ...
                                        <button onclick='window.open("perfil-pri-alumno.php?id=<?= encriptar($idAlum).'&peri='.$tablaPeriodo.'&gra='.$gradoVer.'&sec='.$secciVer.'&nomP='.$nombre_periodo ?>")' type="button" title='Editar ficha del estudiante' class="btn btn-info btn-circle" ><i class="fas fa-user-edit fa-lg" ></i></button>
...
<script type="text/javascript">
    ...
    function borraAlum(id,lin,ced) {
        Swal.fire({
        title: 'Borrar?',
        text: "Elimina completamente el alumno de todas las bases de datos",
        icon: 'warning',
