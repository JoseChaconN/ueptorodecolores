...
$link = Conectarse();
$guarda = (isset($_GET['guarda'])) ? $_GET['guarda'] : 'actual' ;
$tablaPeriodo = (isset($_GET['peri'])) ? $_GET['peri'] : '' ;
$grado = (isset($_GET['gra'])) ? $_GET['gra'] : '' ;
$seccion = (isset($_GET['sec'])) ? $_GET['sec'] : '' ;
$periodo = (isset($_GET['nomP'])) ? $_GET['nomP'] : '' ;
if ($guarda=='nuevo') 
{
    $nacion=''; $cedula=''; $miUsuario=''; $clave=''; $nombre=''; $apellido='';
    $sexo=''; $fechanac=''; $locali=''; $municip=''; $estado='';
    $idEstado=''; $pais=''; $direccion=''; $telefono=''; $ced_rep=''; $correo='';
...
}else
{
    ...
    $alumno_query = mysqli_query($link,"SELECT A.*, B.id_edo,B.estado AS nomedo FROM alumcer A, estado B WHERE A.idAlum ='$idAlum' AND A.estado = B.id_edo "); 
    ...
    $repre_query = mysqli_query($link,"SELECT A.*,B.nomparen FROM represe A, parentescos B WHERE A.cedula ='$ced_rep' AND '$parenRep' = B.idparen "); 
    while ($row = mysqli_fetch_array($repre_query))
    {
        $representante = $row['representante'];
        $correo_rep = $row['correo'];
        $tlf_rep = $row['telefono'];
        $dir_rep = $row['direccion'];
        $foto_rep = $row['ruta'];
        $fnac_repre = $row['fnac_repre'];
        $tlf_celu = $row['tlf_celu'];
    }
    $quien_paga_query = mysqli_query($link,"SELECT * FROM emite_pago WHERE id ='$id_quienPaga' ");
    while ($row = mysqli_fetch_array($quien_paga_query))
    {
        $ced_reci=$row['ced_reci'];
        $nom_reci=$row['nom_reci'];
        $dir_reci=$row['dir_reci'];
    }
    $matri_query = mysqli_query($link,"SELECT retiraPagos,grado,idSeccion,desc1,desc2,desc3,desc4,desc5,desc6,desc7,desc8,desc9,desc10,desc11,desc12,desc13,suma_a_pagado FROM notaprimaria".$tablaPeriodo." WHERE idAlumno ='$idAlum' "); 
    $totDesc=0;
    ...
    $pagos_query = mysqli_query($link,"SELECT SUM(A.montoDolar) as monto FROM pagos".$tablaPeriodo." A, conceptos B WHERE A.idAlum ='$idAlum' and A.id_concepto=B.id and B.afecta='S' and A.statusPago='1' GROUP BY A.recibo "); 
    $pagado=0; $pagos=0;
    while ($row = mysqli_fetch_array($pagos_query))
    {
        $pagado=$pagado+$row['monto'];
        $pagos++;
    }
    if ($fechaHoy>=$venceAgosto) {
        $morosida=$morosida-$agosto;
    }
    $totalPeriodo=$totalPeriodo-$totDesc;
    $pendiente=$totalPeriodo-($pagado+$agosto);
    $morosida=$morosida-$pagado;
} ?>
<div class="container-fluid">
    <h1 class="h3 mb-2 text-gray-800">Ficha del Estudiante</h1>
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Datos Principales</h6>
        </div>
        <div class="card-body">
            <form role="form" method="POST" onsubmit="return validacion();" enctype="multipart/form-data" <?php if($guarda=='nuevo'){echo 'action="alumno-nuevo.php"';}else{echo 'action="alumno-actual.php"';} ?> >
                <!--Fotos-->
                <div class="form-row">
                    ...
                    <div class="col-md-12" style="display: none; background-color: #C8E6C9; margin-bottom: 1%;" id="admin_1">
                        <div class="form-row">
                            ...
                            <div class="form-row col-md-4 text-center">
                                ...
                                <div class="col-md-6">
                                    <label>Pagado</label>
                                    <label class="form-control" data-toggle="tooltip" data-placement="top" title="<?= $pagos ?> pagos realizados "><?= number_format(($pagado+$suma_a_pagado),2,',','.').' $' ?></label>
                            ...
                            <div class="form-row col-md-4 text-center">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <label class="form-control"><?= number_format(($pendiente-$suma_a_pagado),2,',','.').' $' ?></label>
                                </div>
                            </div>
                            <div class="form-row col-md-4 text-center">
                                <div class="col-md-6">
                                    <label>Morosidad $</label>
                                    <label class="form-control"><?= number_format(($morosida-$suma_a_pagado),2,',','.').' $' ?></label>
                                </div>        
                                <div class="col-md-6">
                ...
                <div class="form-row" style="background-color: #FFF9C4; padding: 5px;">
                    ...
                    <div class="col-md-3 col-xs-6 col-sm-3" id="divGrado">
                        ...
                        <select name='gra_alu' id='gra_alum' class="form-control">
                            ...
                            $select1 = mysqli_query($link,"SELECT grado, nombreGrado FROM grado".$tablaPeriodo." WHERE grado<61 " );
                ...
                <div class="form-row" style="background-color: #E0E0E0; padding: 5px;">
                    <div class="col-md-3">
                        ...
                        <div class="form-row">
                            <div class="col-md-4">
                                <select name="nacion" id="nacion" class="form-control" >
                ...
                <div class="form-row" style="margin-top:1%;">
                    ...
                    <div class="col-md-3" style="margin-top: 2%;">
                        <button type="button" style="width: 100%;" class="btn btn-info" onclick='window.open("../reportes/planilla.php?cedalum=<?= $cedula ?>&peri=<?= $tablaPeriodo ?>")' <?php if($guarda=='nuevo'){echo  'disabled';} ?>><i class="fas fa-print fa-sm"></i> Imprimir Planilla</button>
                    </div>
                    <div class="col-md-3" style="margin-top: 2%;">
                    ...
                    <div class="col-md-3" style="margin-top: 2%;">
                        <button type="button" onclick='window.open("../factura/facturar.php?id=<?= encriptar($idAlum).'&peri='.$tablaPeriodo.'&gra='.$grado ?>")' <?php if($guarda=='nuevo'){echo  'disabled';} ?> style="width: 100%;" id="btn-factura" class="btn btn-success"><i class="fas fa-dollar-sign fa-sm"></i> Facturar</button>
                    </div>
                </div>
        ...
        if(ced_buscar.length > 7 ){
            $.post('alumno-nuevo-buscar.php',{'ced':ced_buscar},function(data){
                if(data.isSuccessful){
                    $('#idAlum').val(data.id)
                    $('#nacion').val(data.nacion)
                    $('#clave').val(data.clave)
                    $('#nombre').val(data.nombre)
                    $('#apellido').val(data.apelli)
                    if (data.sexo=='M') {document.querySelector('#sexM').checked = true;}
                    if (data.sexo=='F') {document.querySelector('#sexF').checked = true;}
                    $('#fna_alu').val(data.FechaNac)
                    $('#loc_alu').val(data.locali)
                    $('#edo_alu').val(data.estado)
                    $('#muni_alu').val(data.municip)
                    $('#miUsuario').val(data.miUsuario)
                    $('#direccion').val(data.dire)
                    $('#telefono').val(data.tlf)
                    $('#ced_rep').val(data.ced_rep)
                    $('#par_rep').val(data.parentesco)
                    $('#email').val(data.correo)
                    $("#foto_alu").attr("src",data.ruta);
                    $('#pai_alu').val(data.pais)
                    $('#nom_rep').val(data.nomRep)
                    $('#mai_rep').val(data.maiRep)
                    $('#dir_rep').val(data.dirRep)
                    $('#tlf_hab_rep').val(data.tlfRep)
                    $("#foto_nue").attr("src",data.fotRep);
                    $('#tlf_cel_rep').val(data.celRep)
                    //if (data.editable=='N') {document.querySelector('#editN').checked = true;}
                    //document.getElementById("btn-guarda").disabled = true;
                    debe=data.deuda;
                    periDeb=data.periodoDeb;
                    if(debe>0)
                    {
                        Swal.fire({
                        icon: 'info',
                        ...
                        confirmButtonText:
                        '<i class="fa fa-thumbs-up"></i> Entendido',
                        html: 'Estudiante con deuda pendiente ('+debe+'$)<br>Periodo: '+periDeb+'<br>No puede inscribir hasta solventar deuda pendiente'  
                        })
                        document.getElementById("btn-guarda").disabled = true;
                    }else 
                    {
                        ...
                        confirmButtonText:
                        '<i class="fa fa-thumbs-up"></i> Entendido',
                        text: 'Numero de cedula ya registrada en el sistema!'
                        })
                ...
                }else
                {
                    ...
                    $('#tlf_hab_rep').val('')
                    $('#tlf_cel_rep').val('')
                    /*document.querySelector('#editN').checked = false;
                    document.querySelector('#editS').checked = false;*/
