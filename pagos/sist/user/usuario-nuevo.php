...
$user_query = mysqli_query($link,"SELECT idUser FROM user WHERE cedulaUser ='$cedulaUser' "); 
if(mysqli_num_rows($user_query) > 0)
{
	$sale='Fail';
}else
{
	mysqli_query($link,"INSERT INTO user (nacionUser,cedulaUser,claveUser,fechaNacUser,cargoUser,nombreUser,apellidoUser,telefonoUser,emailUser,direccionUser,impresora) VALUES ('$nacionUser','$cedulaUser','$claveUser','$fechaNacUser','$cargoUser','$nombreUser','$apellidoUser','$telefonoUser','$emailUser','$direccionUser','$impresora')") or die ("NO GUARDO USUARIO".mysqli_error());
	$nuevo_query = mysqli_query($link,"SELECT idUser FROM user where cedulaUser='$cedulaUser' ");
