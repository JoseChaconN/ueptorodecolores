...
$recibos_query = mysqli_query($link,"SELECT A.*,B.nombreUser,B.apellidoUser,C.nombrePago, D.nom_banco FROM miscelaneos A, user B,formas_pago C, bancos D WHERE A.recibo='$recibo' and A.emitidoPor=B.idUser and A.operacion=C.id and A.banco=D.cod_banco ");
...
{ 
	...
	$options[$i]=['conce' => $row['concepto'].' ('.$cantidad.' unid.)','bolivar'=>number_format($row['monto'],2,',','.'),'dolar' => number_format($row['montoDolar'],2,',','.')];
	$i++;
}
