...
<html lang="es"><?php
  ...
  $idAlumno=desencriptar($_GET['id']);
  $idMateria=desencriptar($_GET['idMat']);
  $idDocente=$_SESSION['idAlum'];
  
  $alumno_query=mysqli_query($link,"SELECT A.ruta,A.nombre,A.apellido, B.nombremate FROM alumcer A, materiass".$tablaPeriodo." B WHERE A.idAlum='$idAlumno' and B.codigo='$idMateria' "); 
  
  $row=mysqli_fetch_array($alumno_query);
  $fotoAlumno=$row['ruta'];
  $fotoDocente=$_SESSION['fotoAlum'];
  $fotoAlum = (empty($fotoAlumno)) ? '../imagenes/usuario.png' : '../fotoalu/'.$fotoAlumno ;
  $fotoDoc = (empty($fotoDocente)) ? '../imagenes/usuario.png' : 'fotodoc/'.$fotoDocente ;
  $nombreAlum=$row['nombre'].' '.$row['apellido'];
  $materia=$row['nombremate']; ?>
        ...
        <div class="row">
          <div class="row">
            <div class="col-md-3 offset-md-3 col-xs-6 form-group text-center">
              <img src="<?= $fotoDoc ?>" alt="<?= $fotoDoc ?>" class="thumb" />
            </div>
            <div class="col-md-3 col-xs-6 form-group text-center">
              <img src="<?= $fotoAlum ?>" class="thumb" />
            </div>
          </div><?php 
          $chat_query=mysqli_query($link,"SELECT * FROM chat WHERE idAlum='$idAlumno' and id_docente='$idDocente' and id_materia='$idMateria' ORDER BY fecha_chat DESC"); 
          ?>
          <form role="form" method="POST" enctype="multipart/form-data" action="" >
            <div class="row" style="margin-top: 2%;">
              <div class="col-md-12 form-group">
                <textarea class="form-control" rows="2" id="mensaje" placeholder="Escriba aqui su mensaje para <?= $nombreAlum ?>"></textarea>
              </div>
            </div>
            ...
            <input type="hidden" id="idAlu" value="<?= encriptar($idAlumno) ?>">
            <input type="hidden" id="idMat" value="<?= encriptar($idMateria) ?>">
          </form>
          <div class="col-md-12" style="background-color:#1266F1; color: white; text-align: center; margin-top: 1%;"><h3>Historial de mensajes / <?= $materia ?></h3></div>
          <div class="col-md-12 scroll" style="margin-top:1%; ">
            <table><?php
              ...
              { 
                ...
                $quien = ($envia=='1') ? $_SESSION['nomuser'] : $nombreAlum ;
               ?>
                <tr >
                ...
                <?php
                mysqli_query($link,"UPDATE chat SET visto='1' WHERE id_chat='$id_chat' and envia='2' ") or die ("NO ACTUALIZO ".mysqli_error());
              } ?>
            </table>
          </div>
