...
if(!empty($ced_alu) && $permite=='S')
{
	...
	if(mysqli_num_rows($notas_query) > 0)
	{
		...
		mysqli_query($link,"UPDATE notaprimaria".$tablaPeriodo." SET asistencia$lap='$inas', dias_habiles$lap='$dias', observacion$lap='$obse' WHERE id_notas='$id_notas' ") or die ("NO SE ACTUALIZO".mysqli_error());	
	}else
	{
		mysqli_query($link,"INSERT INTO notaprimaria".$tablaPeriodo." (ced_alu, idAlumno, grado, idSeccion, asistencia$lap, dias_habiles$lap, statusAlum, escola, observacion$lap) VALUES ('$ced_alu', '$idAlum', '$grado', '$seccion','$inas', '$dias', '1', '1', '$obse')") or die ("NO SE GUARDO".mysqli_error());
	}
