...
<html lang="es">
  ...
  $tareas_query = mysqli_query($link,"SELECT A.*, B.nombremate, C.nombre, C.apellido, D.nombreGrado, E.nombre as nomSec FROM tareas".$periodoAlum." A, materiass".$periodoAlum." B, alumcer C, grado".$periodoAlum." D, secciones E WHERE A.lapsoTarea='$lapso' and A.codGrado='$idGra' and A.codSecci='$idSec' and A.codMater=B.codigo and A.cedProf=C.cedula and A.fechaPublica<='$fechahoy' and A.codGrado=D.grado and A.codSecci=E.id  ORDER BY A.fechaPublica DESC"); ?>
  ...
  <main id="main">
    ...
    <div class="breadcrumbs" data-aos="fade-in">
      <div class="container">
        ...
        <div class="col-md-12 col-12">
          <button type="button" onclick='window.open("contactoDoc.php")' class="btn btn-primary" title="Enviar mensaje al docente"><i class="fas fa-comments fa-2x"></i> Enviar Mensaje al Docente</button>
        </div>
          ...
          <table id="table_id" class="table table-striped table-hover">
            ...
            <tbody>
              ...
              { 
                ...
                $nomSimple=$row['nombreArchivo'];
                $tipo=$row['tipo'];
                $id_documento=$row['idTarea'];
                $nomMate=$row['nombremate'];
                ...
                $pasa=1;
                if($todos=='N'){
                  $dirigida_query = mysqli_query($link,"SELECT id_tabla FROM tarea_ind_".$periodoAlum." WHERE idAlum='$idAlum' and idTarea='$id_documento' ");
                ...
                if($pasa==1){
                  ...
                  <tr <?php if ($sinVer==0) { echo 'style="background-color:#CE93D8"'; } ?> > 
                    ...
                    <td><?= $titulo; ?></td>
                    <td style="width: 15%;">
                      <button type="button" onclick="verTarea('<?= $son ?>','<?= encriptar($id_documento) ?>','<?= $nomGra ?>','<?= $nomMate ?>','<?= $docente ?>','<?= $nomarch ?>','<?= $titulo ?>','<?= $descriTarea ?>','<?= $desde ?>','<?= $hasta ?>','<?= $lap ?>','<?= $tipo ?>','<?= $nomSimple ?>')" data-bs-toggle="modal" data-bs-target="#verTarea" class="btn btn-outline-success" title="Ver contenido"><i class="ri-eye-line"></i></button></a>
                    </td>
                  </tr><?php 
                }
              } ?>
          ...
          <div class="modal-body">
            <div class="col">
              <div class="row">
                ...
                <div class="col-md-12 text-center col-xs-12 col-sm-12" style="border-style: ridge; margin-top: 2%;" id="divFrame">
                ...
                <div class="col-md-12" id="divMensaje" style="margin-top:2%;">
                  <h3>Archivo descargado a su dispositivo:</h3>
                  <p id="msjDescarga"></p>
                </div>
  ...
  <script type="text/javascript">
    ...
    function cambiaLap() {
      lap=$('#lapso').val()
      location.href="list-tareas.php?lapso="+lap
    }
    function verTarea(lin,id,grad,mate,prof,arch,titu,desc,desd,hast,laps,tip,nomS) 
    {
      $('#linVer').val(lin);
      $('#idTareaVer').val(id);
      $('#gradoVer').val(grad);
      ...
      if(tip=='application/pdf'){
        ...
        $('#divFrame').hide();
        document.querySelector('#msjDescarga').innerText = nomS;
        $('#divMensaje').show();
      }
      idAlu=$('#idAlu').val()
